...
// by accounting properly for triangular faces.  
// For weighted networks the concept of parallelism 
// has been considered.
// -------------------------------------------------
			...
			t0+= (NW[i] + NW[j])/C[i][j]; 		
				...
				for(int k=0;k<n;k++)
				{
					if(C[i][k]>0.0)
					{
						if(k!=j)
						{
							// Contribution only if no triangle
							if(C[j][k]==0.0)
							{							
							fc -= NW[i]/sqrt(C[i][j]*C[i][k]);
							t1 += NW[i]/sqrt(C[i][j]*C[i][k]);
							}
							else
							{
								double max_side=0.0, sum_side=(C[i][j] + C[j][k] + C[i][k])/2.0;
								if(C[i][j]>=C[i][k]) { max_side = C[i][j]; }
								else                 { max_side = C[i][k]; }
			  					if(C[j][k]>=max_side){ max_side = C[j][k]; }
								...
								{					
									...
									wf = sqrt(sum_side*(sum_side-C[i][j])*(sum_side-C[i][k])*(sum_side-C[j][k]))/sqrt(1.5*0.5*0.5*0.5);
									fc += 0.5 * (C[i][j]/wf);
									t2 += 0.5 * (C[i][j]/wf);
								}
								else
